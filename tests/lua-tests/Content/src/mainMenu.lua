...
local _allTests = {
    ...
    { isSupported = true,  name = "Particle2DTest"           , create_func   =              ParticleTest      },
    { isSupported = true,  name = "Particle3D (PU)"        , create_func   =              Particle3DTest  },
    { isSupported = true,  name = "Physics2DTest"            , create_func =          PhysicsTest  },
...
function CreateTestMenu()
    ...
    local function onPointerMove(event)
        if not event:isPrimaryCaptured()  then
            return
        end
        ...
        if nextPosy > ((TESTS_COUNT + 1) * LINE_SPACE - canvasSize.height) then
            MainMenu:setPosition(0, ((TESTS_COUNT + 1) * LINE_SPACE - canvasSize.height))
            return
        end
